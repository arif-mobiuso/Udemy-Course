...
app.get("/filter" , (req, res)=>{
  const type = req.query.type ;
   jokes = jokes.filter(joke => joke.jokeType === type) ;
...
app.put("/jokes/:id" , (req, res)=>{
  const id = parseInt(req.params.id) ; 
   ...
   jokes[searchIndex-1] = replacementJoke;
   console.log(jokes); 
  res.status(200).send(replacementJoke);
})
...
app.delete("/all" , (req, res)=>{
  ...
  if(userKey === masterKey){
    ...
    res.status(401).send("You are not authorized to do this action")
  }
})
